...
export default function TreeList({ data }) {
  const { infoRoleSelected, openModalEdit, openModalDelete, handleContentSelect } = HandleParent();
      ...
      title: (
        <div key={item._id} className={styles.selectedWrap}>
          <div className={styles.selectedContent}>
            <span
              className={`${infoRoleSelected._id === item._id ? styles.activeRole : ''} ${styles.selectedName} `}
              onClick={() => {
                hasPermission([PERMISSIONS.LIST.LIST_PERMISSION]) && handleContentSelect(item);
              }}
            >
              {item.name}{' '}
            </span>
