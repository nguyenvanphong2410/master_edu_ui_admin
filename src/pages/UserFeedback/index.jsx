...
const UserFeedbackManagement = () => {
  ...
  return (
    <MainLayout>
      ...
      {
        hasPermission([PERMISSIONS.ADD.ADD_CONFIG_FEEDBACK]) && <Button
          className={`main-btn-primary absolute top-[33px] right-[22px]`}
          type={'primary'}
          size={'large'}
              ...
              <Card
                ...
                actions={(()=>{
                  ...
                  if(hasPermission([PERMISSIONS.EDIT.EDIT_CONFIG_FEEDBACK])){
                    result.push(<Tooltip key="edit" placement="bottom" title={'Cập nhật'}>
                    <span
                  ...
                  if(hasPermission([PERMISSIONS.DELETE.DELETE_CONFIG_FEEDBACK])){
                    result.push(<Tooltip key="delete" placement="bottom" title={'Xoá'}>
                    <span
                      className="flex justify-center btn-delete"
                      onClick={() => handleShowModalDeleteFeedback(item)}
                    >
